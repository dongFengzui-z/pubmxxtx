<template>
  <div class="pubmxxtx_draft">
     <mt-cell-group>
            ...
            <mt-field label="学院名称" placeholder v-model="student.XY" readonly></mt-field>
            <mt-field label="班级名称" placeholder v-model="student.BJ" readonly></mt-field>
...
//保存草稿
   Savedrafts(i){
      if (this.XXXNType == "") {
            ...
            let newstudent={
              WID: this.student.WID,
                CZZ:this.student.CZZ,
                CZZXM: this.student.CZZXM,
                CZRQ: this.student.CZRQ,
                XM: this.student.XM,
                XH: this.student.XH,
                XYDM: this.student.XYDM,
                XY: this.student.XY,
                BJDM: this.student.BJDM,
                BJ: this.student.BJ,
...
created(){
  Get(api.queryDetailFormModel,{
    ...
    // console.log(res)
    this.student=res.datas.queryDetailFormModel.rows[0];
     (this.XXXNType = this.student.XXXN_DISPLAY),
                (this.XXXNId = this.student.XXXN),
                (this.XXXQType = this.student.XXXQ_DISPLAY),
                (this.XXXQId = this.student.XXXQ),
                (this.BRDH = this.student.BRDH),
                (this.FMXM = this.student.FMXM),
                (this.FMDH = this.student.FMDH),
                (this.XXYYType = this.student.XXYY_DISPLAY),
                (this.XXYYId = this.student.XXYY),
                (this.YJDZ = this.student.YJDZ);
  });
